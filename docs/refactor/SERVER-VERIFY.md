...
重构尚未全部完成：Next 已接管漫画、阅读中心、后台和写作台，其余页面尚未迁移到新版。生产部署只启动 Next 和 API，旧 Astro 不再运行；因此尚未迁移的 URL 会返回新版的 404，不能把这次部署称为全站功能完成。当前仍使用 SQLite，不能删除数据库或上传目录。
...
本次部署不再保留 Astro PM2 服务，也没有旧前端回退入口。出现新版入口问题时，只能先恢复上一份代码和构建产物，再重新执行部署；这不是数据库回滚，不撤销期间写入的数据。切换前务必保留数据库、上传目录、`.env` 和上一版本构建日志。
