...
本项目采用类似 Venera 的前台源驱动结构：探索、搜索、详情、目录和阅读都发生在前台漫画站；后台只管理源配置和已有漫画条目的基础资料。内容源保存在 SQLite 的 `content_search_sources` 设置中，源站请求由后端发起，源文件中的请求头不会暴露给浏览器。
...
源可以可选地提供 `explore`。配置后，前台漫画首页会像 Venera 的 Explore 页面一样，从当前源加载推荐内容：

```json
{
  "explore": {
    "method": "GET",
    "path": "/discover?type={type}&page={page}&limit={limit}",
    "result_path": "data.items",
    "body_type": "json"
  }
}
...
探索结果使用与搜索相同的 `mapping`。支持 `{page}`、`{limit}` 和 `{type}` 变量。没有配置 `explore` 的源仍然可以正常搜索、查看详情和阅读。
...
源配置最多保留 24 个源，每个源最多 12 个自定义请求头；探索、搜索、详情、目录和阅读请求默认超时 10 秒，允许范围为 1～30 秒。
