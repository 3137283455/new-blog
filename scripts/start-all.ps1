...
try {
  $procs = Get-CimInstance Win32_Process -Filter "name = 'node.exe'" |
    Where-Object {
      ...
      $_.CommandLine -like "*next/dist/bin/next*"
    }
} catch {
...
if (-not $SkipBuild) {
  ...
  Write-Host "[boke] building Next frontend..."
...
Write-Host "[boke] starting Next frontend on 3002..."
Start-Process powershell -WindowStyle Hidden -WorkingDirectory $frontend -ArgumentList "-NoExit", "-Command", "`$env:PORT='3002'; npm.cmd run start"

Write-Host ""
Write-Host "Backend:  http://127.0.0.1:3001"
Write-Host "Frontend: http://127.0.0.1:3002"
Write-Host "Admin:    http://127.0.0.1:3002/admin"
