...
export function readingCenter(req:DeviceRequest,res:Response){const books=db.prepare("SELECT 'book' kind,b.id,b.slug,b.title,b.cover,s.position progress,s.updated_at,v.title section_title,c.title chapter_title,('/books/'||b.slug||'/'||v.slug||'/'||c.slug) href FROM reading_states s JOIN books b ON b.id=s.book_id LEFT JOIN book_volumes v ON v.id=s.volume_id LEFT JOIN book_chapters c ON c.id=s.chapter_id WHERE s.user_id=? AND b.deleted_at IS NULL").all(req.deviceUserId!) as any[];const manga=db.prepare("SELECT 'manga' kind,m.id,m.slug,m.title,m.cover,CASE WHEN (SELECT COUNT(*) FROM manga_pages WHERE chapter_id=c.id)>1 THEN CAST(s.page_index AS REAL)/((SELECT COUNT(*) FROM manga_pages WHERE chapter_id=c.id)-1) ELSE 0 END progress,s.updated_at,v.title section_title,c.title chapter_title,('/manga/'||m.slug||'/'||v.slug||'/'||c.slug) href FROM manga_reading_states s JOIN manga_items m ON m.id=s.manga_id LEFT JOIN manga_volumes v ON v.id=s.volume_id LEFT JOIN manga_chapters c ON c.id=s.chapter_id WHERE s.user_id=? AND m.is_active=1").all(req.deviceUserId!) as any[];return success(res,[...books,...manga].sort((a,b)=>String(b.updated_at).localeCompare(String(a.updated_at))))}
