...
const builtInRules = (): ContentSearchRule[] => [
  {
    ...
    page_base: (process.env.BANGUMI_LOL_PAGE_BASE || 'https://bangumi.lol').replace(/\/+$/, ''), page_path: '/subject/{id}',
  ...
  },
  {
    ...
    page_base: (process.env.BANGUMI_OFFICIAL_PAGE_BASE || 'https://bgm.tv').replace(/\/+$/, ''), page_path: '/subject/{id}',
    search: { method: 'POST', path: '/v0/search/subjects?limit={limit}', result_path: 'data', body_type: 'json', body: { keyword: '{query}', filter: { type: ['{type}'] } } },
    detail: { method: 'GET', path: '/v0/subjects/{id}', result_path: '', body_type: 'json' },
    mapping: { id: 'id', title: ['name_cn', 'name'], original_title: 'name', cover: ['images.large', 'images.common', 'images.medium', 'images.small'], rating: 'rating.score', publication: 'date', description: 'summary', type: ['type_name', 'type'], total: ['eps', 'total_episodes'] },
    type_values: { bangumi: 2, manga: 1 },
...
function normalizeRule(raw: any, index: number): ContentSearchRule | null {
  ...
  if (!id || !label || !apiBase || !kinds.length || !searchPath || !detailPath || !pagePath || !searchTemplate.includes('{query}') || !detailTemplate.includes('{id}') || !pagePath.includes('{id}') || !mapping.id || !mapping.title) return null
  ...
  return {
    id, label, enabled: raw?.enabled !== false, kinds: [...new Set(kinds)], api_base: apiBase, page_base: pageBase, page_path: pagePath, headers,
...
async function requestRule(rule: ContentSearchRule, request: ContentSearchRule['search'] | ContentSearchRule['detail'], variables: Record<string, unknown>, baseHeaders: Record<string, string>) {
  ...
  const response = await fetch(url, { method: request.method, headers: { Accept: 'application/json', ...(body ? { 'Content-Type': contentType } : {}), ...rule.headers, ...baseHeaders }, body, signal: AbortSignal.timeout(10000) })
  if (!response.ok) throw new Error(`${rule.label} HTTP ${response.status}`)
  const contentTypeHeader = response.headers.get('content-type') || ''
  if (!contentTypeHeader.includes('json')) throw new Error(`${rule.label} 返回的不是 JSON`)
