...
export function BookDetailPage({ book, volumes }: { book: BookDetail; volumes: BookVolume[] }) {
  ...
  return (
    <article className="book-page" data-book-contents="" data-book-id={book.id} data-book-slug={book.slug}>
      ...
      {!standalone && <section className="book-contents" id="contents"><header><div><small>ALL CHAPTERS</small><h2>目录</h2><p>{volumes.length} 卷 · {chapters.length || book.chapter_count || 0} 章</p></div><label><span aria-hidden="true">⌕</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索卷名或章节" /></label></header>
