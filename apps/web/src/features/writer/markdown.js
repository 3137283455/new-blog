...
function fontFamilyCss(font) {
  return font?.family
    ? `"${cssString(font.family)}", "Microsoft YaHei", "PingFang SC", sans-serif`
    : '';
}
...
function renderInlineMarkdown(value) {
  ...
  return html(withTokens)
    .replace(/@@FONT_SPAN_(\d+)@@/g, (_match, index) => {
      ...
      return item
        ? `<span data-font="${html(item.family)}"${urlAttr}>${renderInlineMarkdown(item.inner)}</span>`
        : '';
    })
    .replace(
      /&lt;span\s+data-font=(?:&quot;|&#39;)([^&]+?)(?:&quot;|&#39;)(?:\s+data-font-url=(?:&quot;|&#39;)([^&]*?)(?:&quot;|&#39;))?&gt;([\s\S]*?)&lt;\/span&gt;/g,
...
function markdownToHtml(markdown) {
  ...
  for (const raw of lines) {
    ...
    if (task || bullet) {
      ...
      out.push(
        `<li>${task ? `<input type="checkbox" disabled ${task[1].toLowerCase() === 'x' ? 'checked' : ''} /> ${renderInlineMarkdown(task[2])}` : renderInlineMarkdown(bullet[1])}</li>`,
      );
      continue;
    }
    if (/^-{3,}$/.test(line.trim())) {
