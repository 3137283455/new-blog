...
export function collectionKey(source: string, externalId: string) {
...
export function findLocalMangaCollection(source: string, externalId: string) {
  return (
    readLocalMangaCollections().find(
      (item) => collectionKey(item.source, item.external_id) === collectionKey(source, externalId),
    ) || null
  );
}
...
export function saveLocalMangaCollection(input: MangaCollection) {
  ...
  if (!item) return [];
  const items = readLocalMangaCollections().filter(
    (entry) =>
      collectionKey(entry.source, entry.external_id) !==
      collectionKey(item.source, item.external_id),
  );
...
export function mergeMangaCollections(...lists: MangaCollection[][]) {
  ...
  lists.flat().forEach((value) => {
    ...
    if (item)
      merged.set(collectionKey(item.source, item.external_id), {
        ...merged.get(collectionKey(item.source, item.external_id)),
        ...item,
      });
  });
...
export async function getPrivateMangaCollections(apiBase: string, token: string) {
  const response = await fetch(`${apiBase}/private/manga/collections`, {
    headers: { 'X-Device-Token': token },
  });
  const json = await response.json().catch(() => ({}));
  if (!response.ok || !Array.isArray(json.data))
    throw new Error(json.message || '无法读取漫画收藏');
  return json.data.map(normalize).filter(Boolean) as MangaCollection[];
}

export async function syncPrivateMangaCollection(
  apiBase: string,
  token: string,
  item: MangaCollection,
) {
  const response = await fetch(`${apiBase}/private/manga/collections`, {
    method: 'PUT',
...
export async function removePrivateMangaCollection(
  apiBase: string,
  token: string,
  id?: number,
  key?: Pick<MangaCollection, 'source' | 'external_id'>,
) {
  const headers = { 'X-Device-Token': token };
  let targetId = id;
  if (!targetId && key) {
    targetId = (await getPrivateMangaCollections(apiBase, token)).find(
      (item) => item.source === key.source && item.external_id === key.external_id,
    )?.id;
  }
  if (!targetId) return;
  const response = await fetch(`${apiBase}/private/manga/collections/${targetId}`, {
    method: 'DELETE',
    headers,
  });
  if (response.ok) return;
  if (response.status === 404 && key) {
    const remote = (await getPrivateMangaCollections(apiBase, token)).find(
      (item) => item.source === key.source && item.external_id === key.external_id,
    );
    if (remote?.id && remote.id !== targetId) {
      const retry = await fetch(`${apiBase}/private/manga/collections/${remote.id}`, {
        method: 'DELETE',
        headers,
      });
      if (retry.ok) return;
