  ...
  return (
    <div className={`manga-source-shell manga-redesign manga-source-shell-${kind}`}>
      <div className="manga-source-layout">
        <aside
          className="manga-context-rail manga-source-context"
          aria-label={`${titleLabel}来源信息`}
        >
          <nav className="manga-context-nav" aria-label="返回入口">
            <a href={home}>
              <span>01</span>返回{titleLabel}架
            </a>
            {kind === 'manga' && (
              <a href="/manga/search">
                <span>02</span>搜索漫画
          ...
          <section className="manga-context-status">
            ...
            <span>
              {chapters.length ? `${chapters.length} 个章节可用` : '等待来源提供目录接口'}
            </span>
          </section>
          <a
            className="manga-context-footer"
            href={item.source_url || home}
            target={item.source_url ? '_blank' : undefined}
            rel={item.source_url ? 'noopener noreferrer' : undefined}
          >
            <span>来源页面</span>
            <small>{item.source_url ? '打开源站 ↗' : '返回架上'}</small>
          </a>
        </aside>
            ...
            <div className="source-detail-copy">
              ...
              <div className="source-detail-actions">
                ...
                {kind === 'manga' && (
                  <CollectionActions
                    key={source + id}
                    item={{ ...item, source, external_id: id }}
                  />
                )}
              </div>
            </div>
