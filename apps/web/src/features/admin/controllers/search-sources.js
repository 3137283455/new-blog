import { parseMangaSourceFile, mangaSourceBackup } from '../source-files';
export function mount(scope) {
  (() => {
    ...
    function renderVenera() {
      ...
      if (badge) {
        badge.textContent = repositories.length
          ? `${repositories.length} 个仓库 · ${sources.length} 个源`
          : '尚未同步';
      ...
      if (select)
        select.innerHTML = sources
          .map(
            (source) =>
              `<option value="${escapeHtml(source.id)}">${escapeHtml(source.label)} · ${escapeHtml(source.key)}</option>`,
          )
          .join('');
    }
    async function loadVenera() {
      ...
      try {
        ...
        veneraMessage(error.message || 'Venera 源加载失败', true);
      }
    }
    async function importVeneraRepository() {
      ...
      try {
        ...
        state.venera = await api('/admin/venera-sources/import', {
          method: 'POST',
        ...
        if (scope.disposed) return;
        veneraMessage(error.message || '同步失败', true);
      } finally {
    ...
    async function testVeneraSource() {
      ...
      try {
        ...
        const data = await api('/admin/venera-sources/test', {
          method: 'POST',
        ...
        results.innerHTML =
          data.items
            .map(
              (item) =>
                `<article class="grid grid-cols-[3rem_1fr] gap-2 rounded-xl bg-base-200/60 p-2">${item.cover ? `<img class="h-16 w-12 rounded-lg object-cover" src="${escapeHtml(item.cover)}" alt="">` : '<span class="h-16 w-12 rounded-lg bg-base-300"></span>'}<span class="min-w-0"><b class="block truncate text-sm">${escapeHtml(item.title)}</b><small class="block truncate text-base-content/45">${escapeHtml(item.author || item.subtitle || item.external_id || '')}</small></span></article>`,
            )
            .join('') || '<p class="text-sm text-base-content/45">源请求成功，但没有搜索结果。</p>';
    ...
    async function removeVeneraRepository(url) {
      ...
      try {
        ...
        if (scope.disposed) return;
        veneraMessage(error.message || '移除失败', true);
      }
    }
    ...
    function renderList() {
      ...
      const sources = (state.config.sources || []).filter(
        (source) => Array.isArray(source.kinds) && source.kinds.includes('manga'),
      );
      list.innerHTML =
        sources
          .map((source) => {
        ...
        <div class="mt-3 flex flex-wrap gap-2 border-t border-base-content/10 pt-2">
          ...
          .join('') ||
        '<p class="py-8 text-center text-sm text-base-content/45">还没有导入其他漫画源；通常直接使用上方 Venera 仓库即可。</p>';
    }
    function render() {
      renderList();
    }
    async function load() {
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent('content-search-sources-updated', { detail: state.config }),
        );
      } catch (error) {
        if (scope.disposed) return;
        message(error.message || '检索源加载失败', true);
      }
    }
    function conflictChoice(source) {
      const dialog = $('#search-source-conflict-dialog');
      if (!dialog) return Promise.resolve({ mode: 'skip' });
      $('#search-source-conflict-text').textContent =
        `ID“${source.id}”已存在。请选择如何处理“${source.label || source.id}”。`;
    ...
    async function importOne(file, successText = '检索源已保存', presetChoice = null) {
      const sourceId = String(file.source.id || '')
        .trim()
        .toLowerCase();
      ...
      const saved = await api('/admin/search-sources/import', {
        method: 'POST',
    ...
    async function importFiles(files) {
      ...
      try {
        for (const file of files) {
          ...
          const plan = parseMangaSourceFile(parsed);
          skipped += plan.skipped;
          if (plan.venera) {
            if (
              !confirm(
                `导入 ${plan.venera.config.repositories.length} 个 Venera 仓库？同地址仓库配置会更新，其他仓库与书架保留。`,
              )
            )
              continue;
            state.venera = await api('/admin/venera-sources/import', {
              method: 'POST',
              body: JSON.stringify({ file: plan.venera, mode: 'merge' }),
            });
            repositories += plan.venera.config.repositories.length;
            renderVenera();
          }
          for (const sourceFile of plan.sources) {
            if (await importOne(sourceFile, '')) imported += 1;
            else skipped += 1;
          }
        }
        message(
          `导入完成：${repositories} 个 Venera 仓库，新增或覆盖 ${imported} 个接口漫画源${skipped ? `，跳过 ${skipped} 个` : ''}`,
        );
      } catch (error) {
        if (scope.disposed) return;
        message(
          `导入中止：${error.message}；此前已导入 ${repositories} 个仓库、${imported} 个接口源，未撤销。`,
          true,
        );
      }
    }
    function download(data, filename) {
      ...
      scope.defer(() => URL.revokeObjectURL(url));
      link.href = url;
    ...
    async function toggleSource(id) {
      ...
      const next = {
        ...state.config,
        sources: state.config.sources.map((item) =>
    ...
    async function removeSource(id) {
      const source = state.config?.sources.find((item) => item.id === id);
      if (!source || !confirm(`删除检索源“${source.label}”？此操作不会删除追番或漫画数据。`))
        return;
      try {
        state.config = await api(`/admin/search-sources/${encodeURIComponent(id)}`, {
          method: 'DELETE',
        });
        render();
        message('检索源已删除');
    ...
    scope.listen(document, 'click', (event) => {
      ...
      if (!target) return;
      if (
        target.dataset.panelTab === 'search-sources' ||
        target.dataset.panel === 'search-sources'
      ) {
        scope.query('#bangumi-source-dialog')?.close();
      }
      if (target.dataset.veneraRepositoryDelete)
        removeVeneraRepository(target.dataset.veneraRepositoryDelete);
    ...
    scope.listen($('#search-source-conflict-skip'), 'click', () =>
      finishConflict({ mode: 'skip' }),
    );
    scope.listen($('#search-source-conflict-replace'), 'click', () =>
      finishConflict({ mode: 'replace' }),
    );
    scope.listen($('#search-source-conflict-cancel'), 'click', () =>
      finishConflict({ mode: 'cancel' }),
    );
    scope.listen($('#search-source-conflict-rename'), 'click', () => {
      const newId = $('#search-source-conflict-new-id').value.trim().toLowerCase();
      if (!/^[a-z0-9_-]{1,40}$/.test(newId)) {
        message('新源 ID 格式不正确', true);
        return;
      }
      finishConflict({ mode: 'rename', newId });
    });
    scope.listen($('#search-source-conflict-dialog'), 'cancel', (event) => {
      event.preventDefault();
      finishConflict({ mode: 'cancel' });
    });
    scope.listen($('#search-source-export-all'), 'click', async (event) => {
      const button = event.currentTarget;
      button.disabled = true;
      try {
        const [config, venera] = await Promise.all([
          api('/admin/search-sources'),
          api('/admin/venera-sources/export'),
        ]);
        download(mangaSourceBackup(config, venera), 'manga-sources.backup.json');
        message('已导出 Venera 仓库与接口漫画源，不包含源登录会话。');
      } catch (error) {
        if (!scope.disposed) message(error.message || '导出失败', true);
      } finally {
        button.disabled = false;
      }
    });
    scope.listen($('#search-source-import'), 'change', (event) => {
      importFiles(Array.from(event.target.files || []));
      event.target.value = '';
    });
    scope.listen($('#venera-repository-import'), 'click', importVeneraRepository);
    scope.listen($('#venera-source-test'), 'click', testVeneraSource);
    scope.listen($('#venera-source-test-query'), 'keydown', (event) => {
      if (event.key === 'Enter') {
        event.preventDefault();
        testVeneraSource();
      }
    });
    scope.listen(window, 'content-search-sources-request', () => {
      load();
      loadVenera();
    });
    scope.defer(() => finishConflict({ mode: 'cancel' }));
  })();
}
