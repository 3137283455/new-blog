...
function normalizeItem(item, category = DEFAULT_CATEGORY) {
  ...
  if (!url) return null;
  const title =
    String(item?.title || item?.name || url)
      .replace(/\s+/g, ' ')
      .trim() || url;
  return {
    title,
...
export function parseNetscapeBookmarks(source) {
  ...
  const tokens =
    String(source || '').match(
      /<h3\b[^>]*>[\s\S]*?<\/h3\s*>|<dl\b[^>]*>|<\/dl\s*>|<a\b[\s\S]*?<\/a\s*>|<dd\b[^>]*>[\s\S]*?(?=<dt\b|<dl\b|<\/dl\s*>|<h3\b|$)/gi,
    ) || [];
...
function bookmarkUrlIssue(value) {
  ...
  if (/^(mailto:|tel:)/i.test(url))
    return url.includes(':') && url.split(':').slice(1).join(':').trim() ? '' : '链接内容为空';
  if (/^(\/|#)/.test(url)) return '';
  return '不支持此链接协议';
}
...
export function parseBrowserBookmarks(source, fileName = '') {
  ...
  if (looksJson) {
    try {
      ...
      throw new Error('JSON 书签文件无法解析，请重新从浏览器导出');
    }
  } else {
