export function mount(scope) {
  (() => {
    ...
    function render() {
      ...
      list.innerHTML =
        state.items
          ...
          .join('') ||
        '<p class="text-sm text-base-content/45">暂无后台条目。网络漫画请直接从前台搜索；书架和收藏也在前台管理。</p>';
    }
    function openSettings(item) {
      ...
      $('#manga-settings-meta').textContent =
        item.library_type === 'local' ? '本地漫画条目' : '兼容网络条目';
    ...
    scope.listen(document, 'click', async (event) => {
      ...
      if (!target) return;
      if (target.id === 'manga-settings-close' || target.id === 'manga-settings-cancel') {
        $('#manga-settings-dialog')?.close();
        return;
      }
      try {
        if (target.dataset.panelTab === 'manga') await load();
        if (target.dataset.mangaSettings)
          openSettings(
