...
export function mount(scope) {
  (() => {
    ...
    const state = {
      ...
      bangumiSourceItems: [],
      bangumiSourcePage: 0,
      bangumiSearchSource: 'bangumi_lol',
      bangumiPlaySources: [],
      activeAlbumId: null,
      mediaPicker: null,
      mediaPickerItems: [],
    ...
    async function api(path, options = {}) {
      ...
      let res;
      try {
        res = await scope.fetch(`${apiBase}${path}`, { ...options, headers });
      } catch {
      ...
      if (!res.ok || json.success === false)
        throw new Error(json.message || `接口请求失败（HTTP ${res.status}）`);
      return json;
    }
    ...
    async function loadSearchEngines() {
      ...
      state.searchEngines =
        Array.isArray(engines) && engines.length ? engines : [...DEFAULT_SEARCH_ENGINES];
      renderSearchEngines();
    }
    async function saveSearchEngines(message) {
      ...
      await api('/admin/settings', {
        method: 'PUT',
        body: JSON.stringify({ settings: { nav_search_engines: state.searchEngines } }),
      });
    ...
    function applyBangumiSearchConfig(config) {
      ...
      (config.sources || []).forEach((source) => {
        BANGUMI_SEARCH_SOURCE_LABELS[source.id] = source.label;
      });
    ...
    function renderBangumiSearchSource(message = '') {
      ...
      if (status)
        status.textContent =
          message ||
          `当前使用设置中的默认源：${BANGUMI_SEARCH_SOURCE_LABELS[state.bangumiSearchSource] || state.bangumiSearchSource || '未配置'}`;
    }
    async function loadBangumiSearchSource() {
      const json = await api('/admin/search-sources');
      applyBangumiSearchConfig(json.data);
    }
    scope.listen(window, 'content-search-sources-updated', (event) =>
      applyBangumiSearchConfig(event.detail),
    );
    ...
    function setTargetValue(config, value) {
      const form = scope.byId(config.form);
      ...
      if (
        config.form === 'article-form' &&
        config.field === 'cover_image' &&
        typeof updateCoverPreview === 'function'
      ) {
    ...
    function renderMediaPicker() {
      ...
      if (!grid) return;
      const keyword = String($('#media-picker-search')?.value || '')
        .trim()
        .toLowerCase();
      ...
      const visibleItems = state.mediaPickerItems.filter((file) => {
        ...
        return (
          ...
          (!type || file.mime_type?.startsWith(`${type}/`))
        );
      });
      grid.innerHTML =
        visibleItems
    ...
    async function openMediaPicker(config) {
      ...
      $('#media-picker-hint').textContent = config.append
        ? '选择后会追加到当前字段。'
        : '选择后会自动写入当前字段。';
      ...
      try {
        ...
        if (scope.disposed) return;
        $('#media-picker-grid').innerHTML =
          `<p class="text-error">${html(error.message || '媒体库读取失败')}</p>`;
      }
    }
    ...
    function updateFieldPreview(formId, fieldName) {
      ...
      let preview = scope.byId(previewId);
      if (!preview) {
    ...
    function setupCollectionDialogs() {
      ...
      const navigationHeader = $('#navigation-list')
        ?.closest('.ryu-card')
        ?.querySelector('.admin-collection-header');
    ...
    function renderAlbumPhotos() {
      ...
      $('#album-photos-meta').textContent =
        `${photos.length} 张照片 · ${album.location || '未设置地点'}`;
      grid.innerHTML =
        photos
          .map(
            (photo) => `
    ...
    `,
          ...
          .join('') ||
        '<div class="admin-collection-empty">这个相册还没有照片，点击“上传图片”添加第一张。</div>';
    }
    ...
    async function saveNavigationOrder() {
      const ids = Array.from(
        $('#navigation-list')?.querySelectorAll('[data-navigation-drag-id]') || [],
      ...
      try {
        ...
        scope.notify('导航排序已保存');
      } catch (error) {
        if (scope.disposed) return;
        await loadNavigation().catch(() => {});
        scope.notify(error.message || '保存排序失败', true);
      }
    }
    function setupNavigationDrag() {
      ...
      scope.listen(list, 'dragstart', (event) => {
      ...
      scope.listen(list, 'dragover', (event) => {
      ...
      scope.listen(list, 'drop', (event) => {
        if (!list.querySelector('.is-dragging')) return;
        event.preventDefault();
        saveNavigationOrder();
      });
      scope.listen(list, 'dragend', () =>
    ...
    function filteredBookmarkImportRows() {
      ...
      return state.bookmarkImportRows.filter((row) => {
        const matchesQuery =
          !query || `${row.title} ${row.url} ${row.category}`.toLowerCase().includes(query);
        const matchesStatus =
    ...
    function updateBookmarkImportSummary() {
      ...
      if (summary)
        summary.innerHTML = `<span><b>${total}</b> 已识别</span><span class="is-valid"><b>${valid}</b> 可导入</span><span class="is-invalid"><b>${invalid}</b> 有问题</span><span class="is-selected"><b>${selected}</b> 已选择</span>`;
      const confirmButton = $('#bookmark-import-confirm');
      if (confirmButton) {
        confirmButton.disabled = !selected || state.bookmarkImportBusy;
        confirmButton.textContent = state.bookmarkImportBusy
          ? '正在导入…'
          : `导入所选（${selected}）`;
      }
    }
    ...
    async function previewNavigationBookmarks(file) {
      ...
      state.bookmarkImportRows = validateBookmarkItems(
        items,
        state.navigation.map((item) => item.url),
      );
    ...
    async function importSelectedNavigationBookmarks() {
      const selected = state.bookmarkImportRows
        .filter((row) => row.valid && row.selected)
        .map(({ title, url, description, category, icon }) => ({
          title,
          url,
          description,
          category,
      ...
      try {
        for (let index = 0; index < selected.length; index += batchSize) {
          ...
          if (message)
            message.textContent = `正在导入 ${Math.min(index + batch.length, selected.length)} / ${selected.length}…`;
          const json = await api('/admin/navigation/import', {
            method: 'POST',
        ...
        scope.notify(`已导入 ${imported} 个书签${skipped ? `，服务端跳过 ${skipped} 个` : ''}`);
      } catch (error) {
        if (scope.disposed) return;
        if (message) message.textContent = error.message || '导入书签失败';
        scope.notify(error.message || '导入书签失败', true);
      } finally {
        state.bookmarkImportBusy = false;
        renderBookmarkImportPreview();
      }
    ...
    `,
          ...
          .join('') ||
        '<div class="bangumi-play-source-empty">尚未添加播放源，前台将显示“未知”。</div>';
      list
        .querySelectorAll(
          '[data-set-play-source-default], [data-unset-play-source-default], [data-edit-play-source], [data-delete-play-source]',
        )
        .forEach((button) => {
    ...
    function openBangumiPlaySourceDialog(index = -1) {
      ...
      form.elements.namedItem('sort_order').value = String(
        source?.sort_order ?? state.bangumiPlaySources.length,
      );
      ...
      dialog.showModal();
      scope.timeout(() => form.elements.namedItem('name')?.focus(), 0);
    }
    ...
    function renderBangumiSourcePage() {
      ...
      <button class="bangumi-source-result" type="button" data-import-bangumi-source="${html(encodeURIComponent(JSON.stringify(item)))}">
        ...
        <span class="min-w-0">
          ...
          <small>${html(item.source_label || BANGUMI_SEARCH_SOURCE_LABELS[item.source] || '番剧数据源')} ${html(item.external_id || '-')} · ${html(item.type || '未知类型')} · ${html(item.season || '未标日期')} · ${Number(item.rating || 0).toFixed(1)}</small>
    ...
    `,
          ...
          .join('') ||
        '<p class="py-10 text-center text-sm text-base-content/45">没有检索到结果</p>';
      ...
      $('#bangumi-source-page').textContent =
        `第 ${state.bangumiSourcePage + 1} / ${totalPages} 页`;
    ...
    function selectedExtraIds(type) {
      return Array.from(scope.queryAll(`[data-extra-select="${type}"]:checked`)).map(
        (input) => input.value,
      );
    }
    async function batchDeleteExtra(type) {
      ...
      if (!ids.length) {
        scope.notify(`请先勾选要删除的${labels[type] || '项目'}`, true);
        return;
      }
      ...
      try {
        ...
        scope.notify(`已删除 ${ids.length} 项${labels[type] || ''}`);
      } catch (error) {
        if (scope.disposed) return;
        scope.notify(error.message || `批量删除${labels[type] || '项目'}失败`, true);
      }
    }
    async function moveExtra(type, id, direction) {
      ...
      const items = [...(collections[type] || [])].sort(
        (a, b) =>
          Number(a.sort_order || 0) - Number(b.sort_order || 0) || Number(b.id) - Number(a.id),
      );
      ...
      try {
        await api(`${paths[type]}/${current.id}`, {
          method: 'PUT',
          body: JSON.stringify({ sort_order: targetOrder }),
        });
        await api(`${paths[type]}/${target.id}`, {
          method: 'PUT',
        ...
        if (scope.disposed) return;
        scope.notify(error.message || '调整排序失败', true);
      }
    }
    ...
    async function searchBangumiSource() {
      ...
      if (results)
        results.innerHTML = '<p class="text-sm text-base-content/50">正在检索数据源...</p>';
      try {
        ...
        if (scope.disposed) return;
        if (results)
          results.innerHTML = `<p class="text-sm text-error">${html(error.message || '数据源检索失败')}</p>`;
      }
    }
    ...
    async function loadAlbums() {
      ...
      if (selector) {
        selector.innerHTML = state.albums
          .map((album) => `<option value="${album.id}">${html(album.title)}</option>`)
          .join('');
      }
      const list = $('#albums-list');
      if (!list) return;
    ...
    scope.listen($('#search-engine-form'), 'submit', async (event) => {
      event.preventDefault();
      ...
      if (!name || !url)
        return setPanelMessage('search-engine-message', '请填写名称和搜索地址', true);
      if (url !== 'site:' && (!/^https?:\/\//i.test(url) || !url.includes('{query}'))) {
        return setPanelMessage(
          'search-engine-message',
          '外部搜索地址必须以 http(s) 开头并包含 {query}',
          true,
        );
      }
      if (state.searchEngines.length >= 16)
        return setPanelMessage('search-engine-message', '最多添加 16 个搜索来源', true);
      if (state.searchEngines.some((engine) => engine.url === url))
        return setPanelMessage('search-engine-message', '这个搜索地址已经存在', true);
    ...
    scope.listen($('#navigation-form'), 'submit', async (event) => {
      event.preventDefault();
      try {
        const form = event.currentTarget;
        const fields = form.elements;
        ...
        await api(id ? `/admin/navigation/${id}` : '/admin/navigation', {
          method: id ? 'PUT' : 'POST',
    ...
    scope.listen($('#bangumi-form'), 'submit', async (event) => {
      event.preventDefault();
      try {
        const form = event.currentTarget;
        const fields = form.elements;
        ...
        await api(id ? `/admin/bangumi/${id}` : '/admin/bangumi', {
          method: id ? 'PUT' : 'POST',
          body: JSON.stringify(payload),
        });
    ...
    scope.listen($('#bangumi-play-source-form'), 'submit', (event) => {
      event.preventDefault();
      ...
      if (source.is_default) {
        state.bangumiPlaySources = state.bangumiPlaySources.map((item) => ({
          ...item,
          is_default: false,
        }));
      }
    ...
    scope.listen($('#album-form'), 'submit', async (event) => {
      event.preventDefault();
      try {
        const form = event.currentTarget;
        const fields = form.elements;
        ...
        await api(id ? `/admin/albums/${id}` : '/admin/albums', {
          method: id ? 'PUT' : 'POST',
          body: JSON.stringify(payload),
        });
    ...
    scope.listen($('#album-photo-form'), 'submit', async (event) => {
      event.preventDefault();
      try {
        const fields = event.currentTarget.elements;
        const id = fields.namedItem('id')?.value;
    ...
    scope.listen(document, 'click', async (event) => {
      ...
      if (target.dataset.extraEditNavigation) {
        const item = state.navigation.find(
          (row) => String(row.id) === target.dataset.extraEditNavigation,
        );
      ...
      if (target.dataset.deleteSearchEngine) {
        ...
        state.searchEngines = state.searchEngines.filter(
          (engine) => engine.id !== target.dataset.deleteSearchEngine,
        );
        try {
      ...
      if (target.dataset.extraDeleteNavigation && confirm('确认删除这个导航吗？')) {
        try {
          await api(`/admin/navigation/${target.dataset.extraDeleteNavigation}`, {
            method: 'DELETE',
          });
          await loadNavigation();
          scope.notify('导航已删除');
          ...
          if (scope.disposed) return;
          scope.notify(error.message || '删除导航失败', true);
        }
      }
      if (target.dataset.extraEditBangumi) {
        const item = state.bangumi.find(
          (row) => String(row.id) === target.dataset.extraEditBangumi,
        );
        if (item) {
          fill($('#bangumi-form'), item, [
            'id',
            'title',
            'original_title',
            'cover',
      ...
      if (target.dataset.extraDeleteBangumi && confirm('确认删除这个追番吗？')) {
        try {
          ...
          if (scope.disposed) return;
          scope.notify(error.message || '删除追番失败', true);
        }
      }
      ...
      if (target.dataset.extraDeleteAlbum && confirm('确认删除这个相册及其照片吗？')) {
        try {
          ...
          if (scope.disposed) return;
          scope.notify(error.message || '删除相册失败', true);
        }
      }
      if (target.dataset.extraDeletePhoto && confirm('确认删除这张照片吗？')) {
        try {
          ...
          if (scope.disposed) return;
          scope.notify(error.message || '删除照片失败', true);
        }
      }
      ...
      if (target.dataset.extraEditPhoto) {
        const photo = (activeAlbum()?.photos || []).find(
          (item) => String(item.id) === target.dataset.extraEditPhoto,
        );
    ...
    setupNavigationDrag();
    renderBangumiPlaySources();
    scope.listen($('#navigation-create'), 'click', () => openNavigationDialog());
    scope.listen($('#navigation-import'), 'click', () => $('#navigation-import-file')?.click());
    scope.listen($('#navigation-export'), 'click', exportNavigationBookmarks);
    scope.listen($('#navigation-import-file'), 'change', async (event) => {
    ...
    const closeBookmarkImport = () => {
      if (!state.bookmarkImportBusy) $('#bookmark-import-dialog')?.close();
    };
    scope.listen($('#bookmark-import-close'), 'click', closeBookmarkImport);
    scope.listen($('#bookmark-import-cancel'), 'click', closeBookmarkImport);
    scope.listen($('#bookmark-import-search'), 'input', (event) => {
      state.bookmarkImportQuery = event.currentTarget.value || '';
      renderBookmarkImportPreview();
    });
    scope.listen($('#bookmark-import-status'), 'change', (event) => {
      state.bookmarkImportStatus = event.currentTarget.value || 'all';
      renderBookmarkImportPreview();
    });
    scope.listen($('#bookmark-import-list'), 'change', (event) => {
    ...
    scope.listen($('#bookmark-import-select-visible'), 'click', () => {
      filteredBookmarkImportRows().forEach((row) => {
        if (row.valid) row.selected = true;
      });
      renderBookmarkImportPreview();
    });
    scope.listen($('#bookmark-import-clear-visible'), 'click', () => {
      filteredBookmarkImportRows().forEach((row) => {
        row.selected = false;
      });
      renderBookmarkImportPreview();
    });
    scope.listen($('#bookmark-import-confirm'), 'click', importSelectedNavigationBookmarks);
    scope.listen($('#navigation-dialog-close'), 'click', () => $('#navigation-dialog')?.close());
    scope.listen($('#album-create'), 'click', () => openAlbumDialog());
    scope.listen($('#album-dialog-close'), 'click', () => $('#album-dialog')?.close());
    ...
    scope.listen($('[data-close-play-source]'), 'click', () =>
      $('#bangumi-play-source-dialog')?.close(),
    );
    scope.listen($('#bangumi-source-open'), 'click', async () => {
      $('#bangumi-source-dialog')?.showModal();
      try {
        await loadBangumiSearchSource();
      } catch (error) {
        if (scope.disposed) return;
        renderBangumiSearchSource(error.message || '检索源设置加载失败');
      }
      scope.timeout(() => $('#bangumi-source-query')?.focus(), 0);
    });
    scope.listen($('#bangumi-source-close'), 'click', () => $('#bangumi-source-dialog')?.close());
    scope.listen($('#bangumi-source-search'), 'click', searchBangumiSource);
    scope.listen($('#bangumi-source-query'), 'keydown', (event) => {
      if (event.key === 'Enter') {
        event.preventDefault();
        searchBangumiSource();
      }
    });
    scope.listen($('#bangumi-source-prev'), 'click', () => {
      state.bangumiSourcePage -= 1;
      renderBangumiSourcePage();
    });
    scope.listen($('#bangumi-source-next'), 'click', () => {
      state.bangumiSourcePage += 1;
      renderBangumiSourcePage();
    });
    scope.listen($('#bangumi-cover-upload'), 'change', async (event) => {
      try {
    ...
    scope.listen($('#navigation-avatar-upload'), 'change', async (event) => {
      try {
        const file = event.currentTarget.files?.[0];
        if (!file) return;
    ...
    scope.listen($('#album-cover-upload'), 'change', async (event) => {
      try {
        const file = event.currentTarget.files?.[0];
        if (!file) return;
    ...
    scope.listen($('#album-photo-upload'), 'change', async (event) => {
      try {
        const file = event.currentTarget.files?.[0];
        if (!file) return;
    ...
    scope.listen($('#album-photo-bulk-upload'), 'change', async (event) => {
      try {
        const file = event.currentTarget.files?.[0];
        ...
        if (scope.disposed) return;
        scope.notify(error.message || '照片上传失败', true);
      }
    });
    scope.listen($('#close-media-picker'), 'click', () => {
      $('#media-picker-dialog')?.close();
    });
    scope.listen($('#media-picker-search'), 'input', renderMediaPicker);
    scope.listen($('#media-picker-type'), 'change', reloadMediaPickerItems);
    [
      ...
      const field = scope.byId(formId)?.elements.namedItem(fieldName);
      scope.listen(field, 'input', () => updateFieldPreview(formId, fieldName));
