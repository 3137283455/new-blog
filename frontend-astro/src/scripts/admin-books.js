(() => {
  ...
  async function previewFiles(files){
    ...
    $('#book-import-preview').innerHTML='<p>正在解析 EPUB，请稍候…</p>'
    try{state.preview=await api('/admin/books/epub/preview',{method:'POST',body:form});renderPreview();notify('预览生成完成，请检查分卷与章节')}catch(error){state.preview=null;renderPreview();notify(error.message,true)}
  }
  ...
  async function editBook(id){
    ...
    ;['id','title','author','slug','cover','description'].forEach((name)=>{form.elements.namedItem(name).value=book[name]||''})
  ...
  $('#book-epub-files')?.addEventListener('change',(event)=>previewFiles(event.target.files))
  $('#book-new')?.addEventListener('click',()=>{const form=$('#book-form');form.reset();form.elements.namedItem('id').value='';$('#book-volume-editor').innerHTML='';$('#book-editor-card').classList.remove('hidden')})
